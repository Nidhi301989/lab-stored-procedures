...
CREATE PROCEDURE GetMovieRentersByCategory(IN categoryName VARCHAR(255))
BEGIN
    SELECT first_name, last_name, email
    FROM customer
    JOIN rental ON customer.customer_id = rental.customer_id
...
CREATE PROCEDURE GetCategoriesWithMinMovies(IN minMovies INT)
BEGIN
    ...
    HAVING COUNT(film.film_id) > minMovies;
END //
